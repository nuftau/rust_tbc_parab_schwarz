mod volumes;
mod differences;
use crate::utils::linalg::{solve_linear, diff, flip_if};
...
fn integrate_one_step(M : usize,
    ...
    let rhs = stack![Axis(0), array![cond_robin],
...
fn integrate_one_step(M : usize,
    ...
    if upper_domain { // Neumann condition: user give derivative but I need flux
        ...
        let cond_robin = Lambda * u_interface + phi_interface
                - Lambda * dt /(1.+dt*c) * 
                    (f[f.len()-1] + u_nm1[u_nm1.len()-1]/dt);
    ...
    let u_n = ( &f + &(&u_nm1 / dt) + &diff(&(&D*&d))/&h
            - (&d_kp1 + &d_km1) * (a / 2.))*dt / (1.+dt*c);

    debug_assert_eq!(u_n.len(), M);
    let u_phi = match upper_domain {
        true =>(u_n[0] - h[0]*d[1]/6. - h[0]*d[0]/3., phi_ret[0]),
        false => (u_n[u_n.len()-1] + h[h.len()-1]*d[d.len()-2]/6. + h[h.len()-1]*d[d.len()-1]/3., phi_ret[phi_ret.len()-1])};
