use ndarray::{Array1, Axis, stack, ArrayView1};
use crate::utils::linalg::{solve_linear, diff};
...
#[allow(dead_code)]
fn integrate_one_step_star(M1 : usize, M2 : usize,
                               h1 : &ArrayView1<f64>,
                               h2 : &ArrayView1<f64>,
                               D1 : &ArrayView1<f64>,
                               D2 : &ArrayView1<f64>,
    ...
    let rhs = stack![Axis(0), array![dirichlet], rhs.view(),
                            array![neumann]];
    ...
    let u1_n = ( &f.slice(s![..M1]) + &u1_on_dt
            + (&D1_kp1*&d1_kp1 - &D1_km1*&d1_km1)/ &h1
            - a * (&d1_kp1 + &d1_km1) / 2. ) * dt / (1.+dt*c);

    let u2_on_dt = &u_nm1.slice(s![M1..]) / dt;
    let u2_n = ( &f.slice(s![M1..]) + &u2_on_dt
            + &(&D2_kp1*&d2_kp1 - &D2_km1*&d2_km1)/h2
            - a * (&d2_kp1 + &d2_km1) / 2. ) * dt / (1.+dt*c);

    assert_eq!(u1_n.len(), M1);
    assert_eq!(u2_n.len(), M2);

    let u1_interface = u1_n[u1_n.len()-1] + h1[h1.len()-1]*d1[d1.len()-2]/6. + h1[h1.len()-1]*d1[d1.len()-1]/3.;
...
#[allow(non_snake_case)]
#[allow(dead_code)]
pub fn get_Y(M : usize, h : &ArrayView1<f64>,
                  D : &ArrayView1<f64>, a : f64,
                  c : f64, dt : f64, Lambda : f64,
                  upper_domain : bool)
        ->  [Array1<f64> ; 3] {
    assert_eq!(h.len(), M);
    assert_eq!(D.len(), M + 1);
    ...
    if upper_domain {
        Y = get_Y_star(1, M, &array![1.0].view(), h, &array![1., D[0]].view(), D,
                                        a, c, dt);
        ...
        Y = get_Y_star(M, 1, h, &array![1.].view(), D, &array![D[0], 1.].view(),
                       a, c, dt);
...
#[allow(non_snake_case)]
#[allow(dead_code)]
fn get_Y_star(M1 : usize, M2 : usize,
              h1 : &ArrayView1<f64>,
              h2 : &ArrayView1<f64>,
              D1 : &ArrayView1<f64>,
              D2 : &ArrayView1<f64>,
    ...
    for &hi in h1.iter() { assert!(hi > 0.); }
    for &Di in D1.iter() { assert!(Di > 0.); }
    for &hi in h2.iter() { assert!(hi > 0.); }
    for &Di in D2.iter() { assert!(Di > 0.); }
    assert!(a >= 0.);
    assert!(c >= 0.);

    let D_minus = stack![Axis(0), D1.slice(s![1..]),
                    D2.slice(s![1..D2.len()-1])];
    let D_plus = stack![Axis(0), D1.slice(s![1..D1.len()-1]),
                    D2.slice(s![..D2.len()-1])];
    let D_mm1_2 = stack![Axis(0), D1.slice(s![..D1.len()-1]),
                    D2.slice(s![..D2.len()-2])];
    let D_mp3_2 = stack![Axis(0), D1.slice(s![2..]),
                    D2.slice(s![1..])];
    ...
    /////// MAIN DIAGONAL
    let dirichlet = array![-dt/(1.+dt*c) * (1./h[0] + a/(2.*D1[0]))
        - h[0]/(3.*D1[0])];
    let neumann = array![1.];
    ...
    //////// RIGHT DIAGONAL
    let dirichlet = array![dt/(1.+dt*c) * (1./h[0] - a/(2.*D1[1]))
        - h[0]/(6.*D1[1])];
