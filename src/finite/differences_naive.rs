...
use ndarray::{stack, Array, Array1, ArrayView1, Axis};

#[allow(non_snake_case)]
#[allow(dead_code)]
pub fn integrate_one_step_star(
    M1: usize,
    M2: usize,
    h1: &ArrayView1<f64>,
    h2: &ArrayView1<f64>,
    ...
    let middle_f = array![
        f1f[f1f.len() - 1] * (-h1f[h1f.len() - 1]) / (-h1f[h1f.len() - 1] + h2[0])
            + f2[0] * h2[0] / (-h1f[h1f.len() - 1] + h2[0])
    ];
    let D = stack(Axis(0), &[D1f.view(), D2.view()]).expect("Cannot stack D1 and D2");
    let h = stack(
        Axis(0),
        &[(Array::zeros(h1f.raw_dim()) - h1f).view(), h2.view()],
    )
    .expect("Cannot stack h1 and h2");
    let f = stack![
        Axis(0),
        f1f.slice(s![..f1f.len() - 1]),
        middle_f,
        f2.slice(s![1..])
    ];
    let M = M1 + M2 - 1;
    ...
    let rhs = stack![
        Axis(0),
        array![dirichlet],
        sum_both_h * (&f.slice(s![1..f.len() - 1]) + &(&u_nm1.slice(s![1..u_nm1.len() - 1]) / dt)),
        array![neumann]
    ];
    ...
    let phi_interface1 = D1[0] / h1[0] * (u1_n[1] - u1_n[0])
        - h1[0] / 2.
            * ((u1_n[0] - u_nm1[M1 - 1]) / dt + a * (u1_n[1]) / h1[0] + c * u1_n[0] - f1[0]);
...
pub fn get_Y(
    ...
    let interface = array![Lambda - D[0] / h[0]];

    let Y_1 = stack![
        Axis(0),
        interface,
        &sum_both_h * c + (&h_mm1 * &D_mp1_2 + &h_m * &D_mm1_2) * 2. / (&h_m * &h_mm1),
        boundary
    ];

    //////// RIGHT DIAGONAL
    let interface = array![D[0] / h[0]];
    let Y_2 = stack(
        Axis(0),
        &[interface.view(), (&D_mp1_2 * (-2.) / &h_m + a).view()],
    )
    .expect("Cannot stack Y_2.");
    ...
    let boundary = array![bd_cond];
    let Y_0 = stack![Axis(0), &D_mm1_2 * (-2.) / &h_mm1 - a, boundary];
...
pub fn get_Y_star(
    M_star: usize,
    h_star: &ArrayView1<f64>,
    D_star: &ArrayView1<f64>,
    a: f64,
    c: f64,
) -> [Array1<f64>; 3] {
    let M = M_star;
    ...
    let Y_2 = stack(
        Axis(0),
        &[dirichlet.view(), (&D_mp1_2 * (-2.) / &h_m + a).view()],
    )
    .expect("Cannot stack Y_2.");

    //////// LEFT DIAGONAL
    let neumann = array![-1. / h[h.len() - 1]];
    let Y_0 = stack![Axis(0), &D_mm1_2 * (-2.) / &h_mm1 - a, neumann];
    assert_eq!(Y_1.len(), M);
    assert_eq!(Y_2.len(), M - 1);
    assert_eq!(Y_0.len(), M - 1);
    let ret: [Array1<f64>; 3] = [Y_0, Y_1, Y_2];
