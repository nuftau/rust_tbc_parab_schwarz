...
fn test_finite_differences_schwarz() {
    ...
    use crate::utils::linalg::{cos, diff, norm, sin};
    ...
    let f2 = T * (1. + c * t)
        + ratio_D
            * (d * a * cos(&(&x2 * d)) + c * sin(&(&x2 * d)) + &D2_x * d * d * sin(&(&x2 * d))
                - D2_prime * d * cos(&(&x2 * d)));
    ...
    let u0 = stack(Axis(0), &[u1_0.view(), u2_0.view()]).expect("cannot stack u1_0 and u2_0");
    ...
    let ret = differences::integrate_one_step_star(
        M1,
        M2,
        &h1.view(),
        &h2.view(),
    ...
    let mut ecart = 0.;
    for _i in 0..30 {
        let Y = Differences::precompute_Y(
            M2,
            &h2.view(),
            &D2.view(),
            a,
        ...
        let ret = Differences::integrate_one_step(
            M2,
            &h2.view(),
            &D2.view(),
            a,
        ...
        let u_np1_schwarz = stack(Axis(0), &[slice_cut_u1.view(), u2_1.view()])
...
fn test_finite_differences_star() {
    ...
    let u0 = stack(Axis(0), &[u0_1.view(), u0_2.view()]).expect("cannot stack u1_0 and u2_0");

    let u1 = stack(
        Axis(0),
        &[
            (sin(&(&slice_cut_x1 * d)) + T * t).view(),
