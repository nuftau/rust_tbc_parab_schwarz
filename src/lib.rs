...
use ndarray::{stack, Array, Array1, ArrayView, ArrayView1, Axis};
...
/// Note: we won't be able to verify the size of h and D...
#[no_mangle]
pub extern "C" fn full_interface_err(
    time_window_len: usize,
    Lambda_1: f64,
    Lambda_2: f64,
    a: f64,
    ...
    unsafe {
        ...
        let D1a = construct_array_1d(D1, sizes.2);
        let D2a = construct_array_1d(D2, sizes.3);
        let ret = interface_errors_rust_nosum(
            time_window_len,
            Lambda_1,
            Lambda_2,
            a,
...
pub fn interface_errors_rust(
    ...
    let all_err: Vec<Array1<f64>> = (1..number_samples + 1)
        .collect::<Vec<usize>>()
        .par_iter()
        .map(
            |seed| -> ndarray::ArrayBase<ndarray::OwnedRepr<f64>, ndarray::Dim<[usize; 1]>> {
                let errors = match dis {
                    Discretization::FiniteVolumes => interface_errors::<finite::Volumes>(
                        time_window_len,
                        *seed as u64,
                        Lambda_1,
                        Lambda_2,
                ...
                stack(
                    Axis(0),
                    &[errors[0].view(), errors[1].view(), errors[2].view()],
                )
                .unwrap()
            },
        )
...
#[allow(non_snake_case)]
pub fn interface_errors_rust_nosum(
    time_window_len: usize,
    Lambda_1: f64,
    Lambda_2: f64,
    a: f64,
...
pub fn rate_rust(
    ...
    let ret: f64 = match dis {
        Discretization::FiniteVolumes => (1..number_samples + 1)
            .collect::<Vec<usize>>()
            .par_iter()
            .map(|seed| {
                let errors = interface_errors::<finite::Volumes>(
                    time_window_len,
                    *seed as u64,
                    Lambda_1,
                    Lambda_2,
            ...
            .sum(),
        Discretization::FiniteDifferences => (1..number_samples + 1)
            .collect::<Vec<usize>>()
            .par_iter()
            .map(|seed| {
                let errors = interface_errors::<finite::Differences>(
                    time_window_len,
                    *seed as u64,
                    Lambda_1,
                    Lambda_2,
    ...
    T: finite::Discretization,
{
    ...
    let neumann = 0.;
    let dirichlet = 0.;
    let Y1 = T::precompute_Y(M1, h1, D1, a, c, dt, &f1, dirichlet, Lambda_1, false);
    let Y2 = T::precompute_Y(M2, h2, D2, a, c, dt, &f2, neumann, Lambda_2, true);
    let mut all_u1_interface = Array::random(time_window_len, Uniform::new_inclusive(-1., 1.));
    let mut all_phi1_interface = Array::random(time_window_len, Uniform::new_inclusive(-1., 1.));
    ...
    for k in 1..3 {
        ...
        for i in 0..time_window_len {
            ...
            let ret_tuple_integrate = T::integrate_one_step(
                M2,
                h2,
                D2,
                a,
