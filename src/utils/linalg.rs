...
use lapack::dgtsv;
use ndarray::{Array, Array1, ArrayBase, ArrayView1, Axis, Data, Slice};
...
*/
pub fn solve_linear(matrix_to_invert: [&ArrayView1<f64>; 3], f: &ArrayView1<f64>) -> Array1<f64> {
    let y = matrix_to_invert;
    assert_eq!(y[0].len(), y[2].len());
    assert_eq!(f.len(), y[1].len());
    assert_eq!(y[1].len(), y[2].len() + 1);
...
/// flips the slice
pub fn flip_if<S>(condition: bool, x: ArrayView1<S>) -> ArrayView1<S> {
    match condition {
